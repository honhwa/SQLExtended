...
/// <summary>
/// Dockable tool window for the Always On monitor. Hosts <see cref="AgMonitorControl"/>.
/// Docked at the bottom and sized wide — the Databases tab carries a lot of columns.
///
/// Registered <c>MultiInstances</c> (see the package's ProvideToolWindow attribute), so several of these can be
/// open at once, each pinned to a different replica. <see cref="MonitorWindows"/> owns instance ids and reuse.
/// </summary>
[Guid("B7E3F1A2-9C4D-4E8F-A1B2-C3D4E5F60007")]
public sealed class AgMonitorToolWindow : ToolWindowPane, IPinnedMonitorPane
{
    private const string BaseCaption = "Always On";

    public AgMonitorToolWindow() : base(null)
    {
        Caption = BaseCaption;

        var control = new AgMonitorControl();

        // With several windows open the caption is the only thing distinguishing their tabs, so the control names
        // the replica it is pinned to here. It starts as the connect target and is refined to the instance's own
    ...
    internal AgMonitorControl Control => Content as AgMonitorControl;
