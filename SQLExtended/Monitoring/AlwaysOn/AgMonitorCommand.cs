...
/// <summary>
/// Command handler for "Always On Monitor" (Ctrl+Alt+A).
///
/// The monitor is <b>pinned</b> to the replica it was opened from rather than following the active query window, so
...
/// monitors — which here also means the natural way to watch a whole availability group is one window per replica,
/// side by side.
/// </summary>
internal sealed class AgMonitorCommand
{
    public static readonly Guid CommandSet = new Guid("a1b2c3d4-e5f6-7890-abcd-123456789abc");
    private const int AgMonitorCommandId = 0x0e00;

    private readonly AsyncPackage _package;
    private static AgMonitorCommand _instance;

    private AgMonitorCommand(AsyncPackage package, OleMenuCommandService commandService)
    {
        _package = package;
        commandService.AddCommand(new MenuCommand(Execute, new CommandID(CommandSet, AgMonitorCommandId)));
    }
    ...
    public static async Task InitializeAsync(AsyncPackage package)
    {
        ...
        if (commandService != null)
            _instance = new AgMonitorCommand(package, commandService);
    }

    private void Execute(object sender, EventArgs e) => Show(_package);

    /// <summary>
    /// Shows the monitor for one replica. Fire-and-forget; never throws into the caller, failures land in the
    /// ActivityLog.
    /// </summary>
    /// <param name="connectionString">
    /// A connection to the replica to pin to. Null means "use the active query window", which is what the keyboard
    ...
    public static void Show(AsyncPackage package, string connectionString = null, string serverLabel = null)
    {
        ...
        _ = package.JoinableTaskFactory.RunAsync(async () =>
        {
            try
            {
                ...
                var (pane, atCap) = await MonitorWindows.AcquireAsync(package, typeof(AgMonitorToolWindow), key);

                var window = pane as AgMonitorToolWindow;
                ...
                if (string.IsNullOrWhiteSpace(connection))
                {
                    // No connection to pin to, and the window we landed on may already be watching a replica. The
            ...
            {
                await package.JoinableTaskFactory.SwitchToMainThreadAsync(package.DisposalToken);
                ActivityLogHelper.LogError(package, "SQLExtended AG Monitor", "Show failed: " + ex);
                System.Diagnostics.Debug.WriteLine($"[SQLExtended] AG Monitor show failed: {ex}");
            }
        });
    }
}
