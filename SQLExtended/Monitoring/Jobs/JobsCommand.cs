...
/// <summary>
/// Command handler for "Agent Jobs" (Ctrl+Alt+J).
///
/// Like the other three dashboards this one is <b>pinned</b> to the server it was opened from rather than following
/// the active query window, so showing it is not simply "show the tool window": a server has to be resolved first
/// and matched against the windows already open. <see cref="MonitorWindows"/> holds those rules and the instance
/// ids, shared by all four monitors.
/// </summary>
internal sealed class JobsCommand
{
    public static readonly Guid CommandSet = new Guid("a1b2c3d4-e5f6-7890-abcd-123456789abc");
    // Not 0x1000 (that is the SQLExtended symbol) and not 0x0b00 (RegroupServersCommand has it, registered
    // in code rather than in the VSCT symbol list). 0x0f10 continues the command range past 0x0f00.
    private const int JobsMonitorCommandId = 0x0f10;

    private readonly AsyncPackage _package;
    private static JobsCommand _instance;

    private JobsCommand(AsyncPackage package, OleMenuCommandService commandService)
    {
        _package = package;
        commandService.AddCommand(new MenuCommand(Execute, new CommandID(CommandSet, JobsMonitorCommandId)));
    }
    ...
    public static async Task InitializeAsync(AsyncPackage package)
    {
        ...
        if (commandService != null)
            _instance = new JobsCommand(package, commandService);
    }
    ...
    /// A connection to the server to pin to — Object Explorer's for the clicked node, say. Null means "use the
    /// active query window", which is what the keyboard shortcut and the menu do.
    /// </param>
    ...
    public static void Show(AsyncPackage package, string connectionString = null, string serverLabel = null)
    {
        ...
        _ = package.JoinableTaskFactory.RunAsync(async () =>
        {
            try
            {
                ...
                var (pane, atCap) = await MonitorWindows.AcquireAsync(package, typeof(JobsToolWindow), key);

                var window = pane as JobsToolWindow;
            ...
            {
                await package.JoinableTaskFactory.SwitchToMainThreadAsync(package.DisposalToken);
                ActivityLogHelper.LogError(package, "SQLExtended Agent Jobs", "Show failed: " + ex);
                System.Diagnostics.Debug.WriteLine($"[SQLExtended] Agent Jobs show failed: {ex}");
            }
        });
    }
}
